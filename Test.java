...
//		String way="\\.";
	 String tel="01.11.1115sadasdas11ddddddde";
			String way="(1)\\1{2}";   //表示把1做成一个组,在通过\1引用组内一样的内容  结果是11匹配
			                          //为了实现规则的复用，用（）将复用的规则封装，成为正则表达式中的组，
			                         //每一个组都有编号，从1开始。通过使用编号就可以复用对应组的规则内容。
						//注意：编号必须在组的后面，即！先封装后调用   组可以通过从左到右计算其开括号来编号
			                       //	String way="(1)\\1+(d)\\2+";  表示11重复+次,(d)\\2+表示叠词(d) , 引用组内一样的内容
			                       //String way="(11\\.)"; 
